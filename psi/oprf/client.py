...
class Client(object):
    ...
    def __init__(self, pair: Pair, r: Sequence[bytes], codewords: int = 128):
        ...
        for i, word in enumerate(r):
            word_arr = bytes_to_arr(codewords_bytes + self._encode(word))
            self._r[i] = word_arr
    ...
    def prepare(self):
        ...
        if self._codewords == 128:
            for i in range(self._codewords):
                ti_bytes = arr_to_bytes(self._t[:, i])
                ui_bytes = arr_to_bytes(u[:, i])
    ...
    def eval(self, i: int) -> bytes:
        ...
        ti = self._t[i, :]
        return SHA256.new(arr_to_bytes(ti)[4:]).digest()
